...
class ModelsService:
    ...
    def get_all_models(self, model_type: Optional[str] = None) -> List[Model]:
        ...
        for model_data in models_data:
            ...
            models.append(model)
        if model_type in (None, "speech_to_text"):
            whisper_model = Model(
                name="Whisper (local)",
                provider="whisper",
                type="speech_to_text",
            )
            whisper_model.id = "whisper-local"
            models.append(whisper_model)
    ...
    def transcribe(self, model: Model, file_path: str) -> Union[str, dict]:
        """
        Transcribe audio using the given speech-to-text model.
        Special-case: if provider == 'whisper', call Whisper API directly.
        """
        if model.provider == "whisper":
            whisper_api = os.environ.get("Speech_To_Text", "https://d0eb917439a9.ngrok-free.app/transcribe")
            url = whisper_api
        ...
        # 🚨 placeholder for all *other* providers
        logger.error(f"Transcription not supported for provider: {model.provider}")
        return ""    
