...
def transcribe_with_whisper(file_path: str) -> str:
    """
    Send audio file to your Whisper API and return the transcription.
    """
    url = "https://d0eb917439a9.ngrok-free.app/transcribe"
    ...
    if response.status_code == 200:
        data = response.json()
        # assuming your API returns {"text": "..."} 
...
def check_available_providers():
    ...
    # provider_status["whisper"] = os.environ.get("Speech_To_Text") is not None
    # provider_status["whisper"] = True  # Use your ngrok API for Whisper
    ...
    provider_status["whisper"] = (
        os.environ.get("Speech_To_Text") is not None
    )
...
# After fetching from AIFactory
esperanto_available_providers["speech_to_text"] = esperanto_available_providers.get("speech_to_text", [])
if "whisper" not in esperanto_available_providers["speech_to_text"]:
    esperanto_available_providers["speech_to_text"].append("whisper")
...
with st.container(border=True):
    ...
    with col1:
        ...
        selected_stt_model = handle_default_selection(
            "Default STT Model",
...
if uploaded_file and selected_stt_model:
    # Save the file temporarily
    temp_path = f"temp_{uploaded_file.name}"
    with open(temp_path, "wb") as f:
        f.write(uploaded_file.getbuffer())

    # Call your ModelsService to transcribe
    transcript = models_service.transcribe(selected_stt_model, temp_path)
    st.text_area("Transcription Result", value=transcript, height=200)
