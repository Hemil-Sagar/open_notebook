...
def add_model_form(model_type, container_key, configured_providers):
    ...
    with st.form(key=f"add_{model_type}_{container_key}"):
        ...
        # ✅ dictionary-safe filtering
        provider_models = [
            m for m in all_models
            if m.get("provider") == provider and m.get("type") == model_type
        ]

        if provider_models:
            # ✅ strip provider prefix when displaying & storing
            model_name_display = [
                m.get("name").split(" - ")[-1] for m in provider_models
            ]
            ...
            # ✅ find the full entry for the selected model
            selected_model = next(
                (m for m in provider_models if m.get("name", "").endswith(selected_display)),
                None
            )
            model_name = selected_display if selected_model else None
        else:
            st.warning("⚠️ No models found for this provider/type.")
            model_name = None

        if st.form_submit_button("Add Model") and model_name:
            st.write("📤 Creating model with:", {
                "name": model_name,
                "provider": provider,
            ...
            try:
                ...
                st.success(f"✅ Model **{provider}/{model_name}** added!")
                st.rerun()
